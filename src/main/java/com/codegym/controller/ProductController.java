...
public class ProductController {
    ...
    @PostMapping("/create")
    private ModelAndView createProduct(@Valid @ModelAttribute ProductForm productForm, BindingResult bindindResult) {
    ...
    private ModelAndView editProduct(@PathVariable Long id){
        Optional<Product> product = this.productService.findById(id);
        if (!product.isPresent()){
            return new ModelAndView("/product/error-404");
        }
